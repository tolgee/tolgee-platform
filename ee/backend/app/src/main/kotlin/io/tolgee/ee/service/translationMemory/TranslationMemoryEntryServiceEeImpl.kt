...
/**
 * Maintains the synced entries on assigned **shared** TMs after every translation save. Shared
 * TMs are paid-only, which is why this implementation lives under `ee/`. The OSS module ships a
 * no-op stub ([io.tolgee.service.translationMemory.TranslationMemoryEntryServiceOssImpl]); this
 * `@Primary`-annotated bean overrides it whenever the EE module is on the classpath.
 *
 * Project TMs receive no stored entries — their content is computed virtually from the project's
 * translations at read time — so this service intentionally ignores PROJECT-type assignments.
 *
 ...
 *   the DB (not [io.tolgee.model.key.Key.translations] in memory — that collection is only
 *   synchronized after each save returns, so a target saved before the base in the same
 *   `setForKey` call would not see it yet).
...
@Service
@Primary
class TranslationMemoryEntryServiceEeImpl(
...
) : TranslationMemoryEntryService {
  @Transactional
  override fun onTranslationSaved(translation: Translation) {
    val key = translation.key
    val projectId = key.project.id

    // Skip the rest cheaply when nobody has writable shared assignments — the common free-plan
    // case. Project TMs always have a writable self-assignment, so we filter explicitly.
    val sharedAssignments =
      translationMemoryProjectRepository
        .findByProjectIdAndWriteAccessTrue(projectId)
