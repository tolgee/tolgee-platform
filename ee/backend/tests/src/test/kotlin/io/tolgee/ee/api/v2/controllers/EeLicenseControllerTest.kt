...
class EeLicenseControllerTest : AuthorizedControllerTest() {
  ...
  fun `releases a license key the remote no longer knows`() {
    ...
    mockReleaseKeyFailure(Message.LICENSE_KEY_NOT_FOUND.code)
  ...
  fun `keeps the subscription when the remote fails for another reason`() {
    ...
    mockReleaseKeyFailure(Message.LICENSE_KEY_USED_BY_ANOTHER_INSTANCE.code)
  ...
  private fun mockReleaseKeyFailure(errorCode: String) {
    eeLicensingMockRequestUtil.mock {
      whenReq {
        this.method = { it == HttpMethod.POST }
        this.url = { it.contains("/v2/public/licensing/release-key") }
