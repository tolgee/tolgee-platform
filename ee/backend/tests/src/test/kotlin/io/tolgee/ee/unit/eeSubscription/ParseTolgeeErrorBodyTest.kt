...
class ParseTolgeeErrorBodyTest {
  @Test
  fun `reads the error code`() {
    assertThat(errorBodyOf("""{"code": "${Message.OUT_OF_CREDITS.code}"}""")?.code)
  ...
  fun `reads the error code when the remote sends fields we do not know`() {
    ...
    assertThat(errorBodyOf(body)?.code).isEqualTo(Message.OUT_OF_CREDITS.code)
  }
  ...
  fun `returns null for a body that is not our error json`() {
    assertThat(errorBodyOf("<html>502 Bad Gateway</html>")).isNull()
  }

  private fun errorBodyOf(body: String) =
