...
/**
 * Verifies that the QA-batch-job leg of [LanguageStatsListener] refreshes language_stats for
 * the branches the job actually touched, including reruns where the underlying entities are
 * intentionally not activity-logged.
 *
 * The QA chunk processor (see `QaCheckBatchServiceImpl` / `QaIssueService`) sets
 * `disableActivityLogging = true` on every entity it persists, so `activity_modified_entity`
 * is empty for these jobs. The listener must therefore derive affected branches from the
 * batch job's targets, not from activity logging.
 *
 * The listener is fired directly with a synthetic [OnBatchJobFinalized] (matching the real
 * payload structure: projectId, type=QA_CHECK, target=List<BatchTranslationTargetItem>) so we
 * can pinpoint the branch-resolution behavior without scheduling a real coroutine batch job.
 */
@SpringBootTest
