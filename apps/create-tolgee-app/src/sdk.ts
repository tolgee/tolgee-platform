...
import { PUBLISHED_SDK_VERSION, SDK_PACKAGE_NAME, type SdkMode } from './registry'
...
/** The SDK sits at `<repo>/apps/tolgee-apps-sdk`. */
const repoRootOf = (sdkDir: string): string => resolve(sdkDir, '..', '..')

const workspaceGlobs = (repoRoot: string): string[] => {
  try {
    const parsed = JSON.parse(
...
/**
 * Whether the generated app would become an npm workspace of the repo the SDK
...
export const isWorkspaceTarget = (
  ...
  const rel = relative(repoRoot, targetDir)
  if (rel.length === 0 || rel.startsWith('..') || isAbsolute(rel)) return false
  const posixRel = rel.split(sep).join('/')
  return workspaceGlobs(repoRoot).some((glob) =>
 ...
 * it by whole APIs. Inside the repo's workspaces a range is enough — npm links
 * the local package — everywhere else the dependency has to name the directory
 * outright, or npm resolves it from the registry instead.
 *
 * Throws in `local` mode when this CLI has no SDK sources next to it.
 */
export const resolveSdk = (input: ResolveSdkInput): SdkResolution => {
  ...
  if (input.mode === 'published' || (input.mode === 'auto' && !sdkDir)) {
    const version =
      (sdkDir && localSdkVersion(sdkDir)) ?? PUBLISHED_SDK_VERSION
    return {
      spec: version,
      source: 'published',
      sdkDir,
  ...
  const repoRoot = repoRootOf(sdkDir)
  const buildRequired = !isBuilt(sdkDir)

  if (isWorkspaceTarget(input.targetDir, repoRoot)) {
    return {
      spec: '*',
      source: 'workspace',
      sdkDir,
      buildRequired,
      summary: `local ${SDK_PACKAGE_NAME} via npm workspaces (${relative(repoRoot, sdkDir)})`,
    }
  }
