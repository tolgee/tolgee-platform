...
describe('Community projects navigation', () => {
  ...
  it('renders exactly one Projects title and the inline switcher on both list pages', () => {
    gcy('projects-list-title').should('have.length', 1).contains('Projects');
    gcy('organization-switch').should('be.visible');
  ...
  it('highlights no org row and hides the community footer while on the community page', () => {
    visitCommunity();
    openSwitch();
  ...
  it('does not offer the community entry from a non-list switcher', () => {
    cy.visit(
...
describe('Community projects list content', () => {
  ...
  it('keeps the search field visible after clearing an active search', () => {
    ...
    // refetch lands — the exact moment a latch-less build would unmount/remount the field. Only
    // then does the focus check discriminate: the latch keeps the input mounted+focused throughout,
    // whereas without it the remounted input is unfocused.
    gcy('dashboard-projects-list-item').should('have.length', 6);
