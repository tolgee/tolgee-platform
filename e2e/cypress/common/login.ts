...
export const loginWithFakeSso = () => {
  cy.intercept('http://authorizationuri/**', {
    statusCode: 200,
    body: 'Fake Sso',
  }).as('sso');
  ...
  cy.wait('@sso').then((interception) => {
    const params = new URL(interception.request.url).searchParams;
    expect(params.get('client_id')).to.eq('clientId');
    expect(params.get('response_type')).to.eq('code');
    expect(params.get('scope')).to.eq('openid profile email offline_access');
    expect(params.get('state')).to.matches(
      /^[0-9a-f]{8}-[0-9a-f]{4}-[0-5][0-9a-f]{3}-[089ab][0-9a-f]{3}-[0-9a-f]{12}$/i
    ); // should be the uuid generated by crypto
