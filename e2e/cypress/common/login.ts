...
const loginWithFakeOAuth2Generic = (
  service: string,
  ...
  cy.contains(loginButtonLabel).click();
  ...
  cy.wait('@oauth2').then((interception) => {
    ...
    cy.contains('Fake OAuth2').should('be.visible');
    cy.visit(
      HOST +
        `/login/auth_callback/${service}?code=this_is_dummy_code&redirect_uri=https%3A%2F%2Fdummy-url.com%2Fcallback&state=${params.get(
          'state'
        )}`
    );
    cy.contains('Projects').should('be.visible');
