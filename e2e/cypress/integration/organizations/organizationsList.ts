...
describe('Organization List', () => {
    ...
    it("creates organization", () => {
        ...
        gcy("organization-name-field").within(() => cy.get("input").type("What a nice organization"))
        gcy("organization-address-part-field").within(() => cy.get("input").should('have.value', "what-a-nice-organization"))
        gcy("organization-description-field").within(() => cy.get("input").type("Very nice organization! Which is nice to create!"))
        clickGlobalSave()
        gcy("global-paginated-list").within(() => cy.contains("What a nice organization"))
    ...
    it("validates creation fields", () => {
        ...
        gcy("organization-name-field").within(() => {
            cy.get("input").type("This is too too too too too too too too too too too too too too too too too too long")
    ...
    describe("list", () => {
        it("contains created data", () => {
            gcy("global-paginated-list").within(() => {
                ...
                cy.contains("ZZZ Cool company 10").scrollIntoView().should("be.visible")
                goToPage(2)
        ...
        it("admin cannot see microsoft settings button", () => {
            gcy("global-paginated-list").within(() => {
                cy.contains("Microsoft").closest("li").within(() => {
        ...
        it("admin leaves Microsoft", {scrollBehavior: 'center'}, () => {
            ...
            gcy("global-paginated-list").within(() => {
                cy.contains("Microsoft").closest("li").within(($li) => {
        ...
        it("admin cannot leave Techfides", {scrollBehavior: 'center'}, () => {
            gcy("global-paginated-list").within(() => {
                cy.contains("Techfides").closest("li").within(($li) => {
                    gcy("leave-organization-button").click()
                })
            })
            gcy("global-confirmation-confirm").click()
        ...
        it("admin can access Tolgee settings", {scrollBehavior: 'center'}, () => {
            gcy("global-paginated-list").within(() => {
                cy.contains("Tolgee").closest("li").within(($li) => {
