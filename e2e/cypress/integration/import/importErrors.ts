import {cleanImportData, generateAllSelectedImportData, generateBaseImportData, generateManyLanguagesImportData, login} from "../../common/apiCalls";
...
describe('Import errors', () => {
    ...
    describe("file error message", () => {
        beforeEach(() => {
            ...
            cy.get("[data-cy=dropzone]")
                .attachFile({filePath: "import/error.jsn", fileName: "error.json"}, {subjectType: 'drag-n-drop'})
        })

        it("shows error for bad file", () => {
            gcy("import-file-error").contains("Cannot parse file").should("be.visible")
        })

        it("error shows more and less", () => {
            gcy("import-file-error").findDcy("import-file-error-more-less-button").click()
            gcy("import-file-error").contains("Cannot construct instance of").should("be.visible")
            gcy("import-file-error").findDcy("import-file-error-more-less-button").click()
            gcy("import-file-error").contains("Cannot construct instance of").should("not.exist")
        })
