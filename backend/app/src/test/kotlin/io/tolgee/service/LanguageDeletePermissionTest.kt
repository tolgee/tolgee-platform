...
class LanguageDeletePermissionTest : AbstractSpringTest() {
  ...
  @Test
  @Transactional
  fun `lowers permissions for suggest (granular)`() {
    checkUser(testData.suggestScopeUser) {
      assertThat(computedPermissions.scopes).doesNotContain(Scope.TRANSLATIONS_SUGGEST)
      assertThat(computedPermissions.scopes)
        .containsExactlyInAnyOrder(Scope.TRANSLATIONS_VIEW, Scope.KEYS_VIEW)
    }
  }
  ...
  @Test
  @Transactional
  fun `keeps suggest scope when a non-last language is deleted (granular)`() {
    checkUser(testData.suggestMultiLangUser) {
      assertThat(computedPermissions.scopes).contains(Scope.TRANSLATIONS_SUGGEST)
      assertThat(computedPermissions.suggestLanguageIds).hasSize(1)
    }
  }
