...
class McpOAuthAccessTest : AbstractMcpTest() {
  ...
  fun setup() {
    ...
    testData = BaseTestData(userName = "mcp_oauth_user", projectName = "mcp_oauth_project")
    otherProjectId =
      testData.root
        .addProject {
    ...
    testData.projectBuilder.addBranch {
      name = "main"
      isDefault = true
    }
  ...
  fun `a project-scoped tool is refused for a token bound to another project`() {
    val token = tokenFor(listOf(Scope.KEYS_VIEW.value, Scope.TRANSLATIONS_VIEW.value), otherProjectId)

    assertToolFails(
      createMcpClientWithBearer(token),
      "list_keys",
      projectArgument(),
      expectedError = "ProjectNotFoundException",
    )
  }
