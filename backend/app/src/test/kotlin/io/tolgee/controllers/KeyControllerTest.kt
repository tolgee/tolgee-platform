...
class KeyControllerTest : AuthorizedControllerTest() {
  ...
  fun create() {
    ...
    assertThat(keyService.find(project.id, PathDTO.fromFullPath("test string"))).isNotEmpty
  }
  ...
  fun editDeprecated() {
    ...
    assertThat(keyService.find(project.id, PathDTO.fromFullPath("test string"))).isEmpty
    assertThat(keyService.find(project.id, PathDTO.fromFullPath("hello"))).isNotEmpty
  }
  ...
  fun delete() {
    ...
    val keyInstance = keyService.find(project.id, PathDTO.fromFullPath(keyDto.key)).orElseGet(null)

    performDelete(projectId = project.id, keyInstance.id!!)

    assertThat(keyService.find(project.id, PathDTO.fromFullPath(keyDto.key))).isEmpty
    assertThat(keyService.find(project.id, PathDTO.fromFullPath(keyDto2.key))).isNotEmpty
  }
  ...
  fun deleteMultiple() {
    ...
    val keyInstance = keyService.find(project.id, PathDTO.fromFullPath(keyDto.key)).orElseGet(null)
    val keyInstance2 = keyService.find(project.id, PathDTO.fromFullPath(keyDto2.key)).orElseGet(null)

    performDelete(projectId = project.id, setOf(keyInstance.id!!, keyInstance2.id!!))

    assertThat(keyService.find(project.id, PathDTO.fromFullPath(keyDto.key))).isEmpty
    assertThat(keyService.find(project.id, PathDTO.fromFullPath(keyDto2.key))).isEmpty
  }
