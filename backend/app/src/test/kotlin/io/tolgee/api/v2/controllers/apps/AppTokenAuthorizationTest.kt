...
class AppTokenAuthorizationTest : AuthorizedControllerTest() {
  ...
  @Test
  fun `is rejected on an endpoint that is not project-scoped`() {
    asApp(
      patch("/v2/organizations/${testData.organization.id}/apps/$installId/manifest-url")
        .contentType(MediaType.APPLICATION_JSON)
        .content("""{"manifestUrl":"https://evil.example.com/manifest.json"}"""),
    ).andIsForbidden.andHasErrorMessage(Message.APP_ACCESS_FORBIDDEN)
  }
  ...
  /**
   * An install whose manifest asks for project.edit clears the scope check that guards the
   * app-management endpoints, so only the app-access rule keeps it from enabling further apps for
   * the project — or disabling the ones already there.
   */
  @Test
  fun `cannot manage apps even when its grant covers project edit`() {
