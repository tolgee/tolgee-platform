...
class V2KeyControllerTest : ProjectAuthControllerTest("/v2/projects/") {
  ...
  fun `creates key with translations and tags and screenshots`() {
    ...
    val key = keyService.find(project.id, keyName).orElseThrow()
  ...
  fun `deletes single key`() {
    ...
    assertThat(keyService.find(testData.firstKey.id)).isEmpty
  }
  ...
  fun `deletes single key with references`() {
    ...
    assertThat(keyService.find(testData.keyWithReferences.id)).isEmpty
  }
  ...
  fun `deletes multiple keys`() {
    ...
    assertThat(keyService.find(testData.firstKey.id)).isEmpty
    assertThat(keyService.find(testData.secondKey.id)).isEmpty
  }
