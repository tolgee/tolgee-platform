...
class KeyComplexEditHelper(
  ...
  private fun languageByTag(tag: String): Language = languages.find { it.tag == tag } ?: throw NotFoundException(Message.LANGUAGE_NOT_FOUND)

  private fun languageById(id: Long): Language = languages.find { it.id == id } ?: throw NotFoundException(Message.LANGUAGE_NOT_FOUND)
