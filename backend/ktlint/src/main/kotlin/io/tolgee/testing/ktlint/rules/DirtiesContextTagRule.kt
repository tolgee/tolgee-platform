 ...
 */
class DirtiesContextTagRule : Rule(RULE_ID, About(maintainer = "Tolgee Team")), RuleAutocorrectApproveHandler {
  override fun beforeVisitChildNodes(node: ASTNode, emit: (Int, String, Boolean) -> AutocorrectDecision) {
    when (val psi = node.psi) {
      is KtNamedFunction -> {
        ...
        val hasDirtiesContext = functionAnnotations.any {
          it.shortName?.asString() == "DirtiesContext"
        }

        if (hasDirtiesContext) {
          val hasClassContextRecreatingTag = psi.containingClass()?.annotationEntries?.any {
      ...
      is KtClass -> {
        ...
        val hasDirtiesContext = annotations.any {
          it.shortName?.asString() == "DirtiesContext"
        }

        if (hasDirtiesContext) {
          val hasContextRecreatingTag = annotations.any {
            it.shortName?.asString() == "ContextRecreatingTest"
          }
