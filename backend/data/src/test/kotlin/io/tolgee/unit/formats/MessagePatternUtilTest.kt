...
class MessagePatternUtilTest {
  ...
  fun `returns correct pattern strings for root node`() {
    ...
    MessagePatternUtil.buildMessageNode(full)
  ...
  fun `returns correct pattern strings simple arg node`() {
    ...
    root.contents.single().patternString.assert.isEqualTo("{hello}")
  }
  ...
  fun `returns correct pattern strings with arg node wrapped with text`() {
    ...
    root.contents[1].patternString.assert.isEqualTo("{hello}")
  }
  ...
  fun `returns correct pattern strings with arg node with type`() {
    ...
    root.contents[1].patternString.assert.isEqualTo("{hello, number}")
  }

  @Test
  fun `returns correct pattern strings with arg node with style`() {
    MessagePatternUtil.buildMessageNode("Hello, {hello, number, scientific}!")
      .contents[1].patternString.assert.isEqualTo("{hello, number, scientific}")
    MessagePatternUtil.buildMessageNode("Hello, {hello, number, 0.00}!")
      .contents[1].patternString.assert.isEqualTo("{hello, number, 0.00}")
  }
  ...
  fun `returns correct pattern string for plural forms`() {
    ...
    argNode.complexStyle!!.variants.let {
      ...
      one.message?.contents!![0].patternString.assert.isEqualTo("I have one dog")
      val other = it[1]
  ...
  fun `returns correct pattern string for nested plurals`() {
    ...
    val secondPlural = (firstPlural.complexStyle!!.variants[0].message?.contents?.get(0) as MessagePatternUtil.ArgNode?)
    val thirdPlural = (firstPlural.complexStyle!!.variants[1].message?.contents?.get(0) as MessagePatternUtil.ArgNode?)
  ...
  fun `returns correct pattern string for escape sequences`() {
    ...
    simple.contents[0].patternString.assert.isEqualTo("'{'Hello'}' ")
    simple.contents[1].patternString.assert.isEqualTo("{name}")

    val comlpex =
      MessagePatternUtil.buildMessageNode(
        "'{Hello}' {name} '{'Lala'}' '{Lala ' '",
      )
    comlpex.contents[0].patternString.assert.isEqualTo("'{Hello}' ")
  ...
  fun `returns correct pattern string for complex plurals`() {
    ...
    val firstPlural = root.contents.first() as MessagePatternUtil.ArgNode
    firstPlural.complexStyle!!.variants[0].patternString.assert.isEqualTo("No books")
    firstPlural.complexStyle!!.variants[1].patternString.assert.isEqualTo("# book (and one other)")
    firstPlural.complexStyle!!.variants[2].patternString.assert.isEqualTo("# books (and # others)")
  }
