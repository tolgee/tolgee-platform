 ...
 * the availability set goes, and the app's credentials go with it.
 *
...
class AppOwnerRemovalService(
  ...
  /**
   * Deleting the app takes `app_secret` and `app_availability` with it through the database's own
   * cascades, which is what revoking the credentials amounts to here and is stronger than marking
   * them revoked. No uninstall is delivered: the app is being taken down, so there is nothing to
   * tell it, and every install simply stops appearing in its discovery call.
   */
  private fun purge(appEntityId: Long): Int {
    ...
    installs.forEach { appEnablementService.removeAllForAppInstall(it.id) }
    appAvailabilityService.removeAllForApp(appEntityId)
