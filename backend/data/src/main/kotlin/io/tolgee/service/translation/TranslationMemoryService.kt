...
/**
 * Translation Memory suggestion service. Queries `translation_memory_entry` for stored entries
 * (synced + manual on shared TMs, manual on project TMs) and computes virtual rows on the fly
 * for project TMs from the project's own translations.
 *
 ...
 *   - Paid plan (`Feature.TRANSLATION_MEMORY`) → project TM + every assigned shared TM with read
 *     access.
 *
 * Every active project gets a project TM (created at project creation, back-filled for legacy
 * projects via Liquibase), so callers don't need a fallback for the "no readable TMs" case.
 */
@Service
