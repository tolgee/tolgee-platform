...
class OAuth2AccessTokenResolver(
  ...
  fun tryResolve(
    ...
    // RFC 8707 audience binding: a token presented to a resource server other than the one its grant was authorized
    // for is invalid there, even though the credential itself is genuine. A grant whose stored audience no longer
    // resolves matches nothing.
    ...
    return TolgeeAuthentication(
      credentials = OAuth2TokenCredentials(grant.issuedTokenScopeSet(), grant.boundProjectIds()),
      deviceId = null,
      userAccount = user,
