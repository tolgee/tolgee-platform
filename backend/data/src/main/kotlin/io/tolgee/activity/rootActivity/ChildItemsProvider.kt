...
class ChildItemsProvider(
  private val activityRevisionId: List<Long>,
  ...
  private fun getItemsRaw(): List<Array<Any?>> {
    return entityManager.createNativeQuery(
      ...
      where ame.entity_class = :entityClass
        and ame.activity_revision_id in :revisionIds
      ...
      where ade.activity_revision_id in :revisionIds
        ...
        and ade.entity_id not in (select ame.entity_id id
                                  from activity_modified_entity ame
                                  where ame.activity_revision_id in :revisionIds
