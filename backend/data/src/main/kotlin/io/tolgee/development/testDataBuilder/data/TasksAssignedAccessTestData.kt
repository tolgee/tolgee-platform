...
class TasksAssignedAccessTestData : BaseTestData("tasksAssignedAccessOwner", "Tasks assigned-access project") {
  ...
  lateinit var taskTranslation: Translation
  ...
  init {
    projectBuilder.apply {
      ...
      // A REVIEW task over the same key, so the state-change gate has an assignee path of its own to exercise.
      val reviewTask =
        addTask {
          number = REVIEW_TASK_NUMBER
          name = "Granular assignee review task"
  ...
  companion object {
    ...
    const val REVIEW_TASK_NUMBER = 3L
