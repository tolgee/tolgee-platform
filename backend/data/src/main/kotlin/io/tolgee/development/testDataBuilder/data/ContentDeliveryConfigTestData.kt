...
class ContentDeliveryConfigTestData : BaseTestData() {
  val azureContentStorage = projectBuilder.addContentStorage {
    this.azureContentStorageConfig = AzureContentStorageConfig(this).apply {
      connectionString = "fake"
      containerName = "fake"
    }
  }

  val s3ContentStorage = projectBuilder.addContentStorage {
    name = "S3"
    this.s3ContentStorageConfig = S3ContentStorageConfig(this).apply {
      bucketName = "fake"
      accessKey = "fake"
      secretKey = "fake"
      endpoint = "fake"
  ...
  val defaultServerContentDeliveryConfig = projectBuilder.addContentDeliveryConfig {
    name = "Default server"
  }

  val azureContentDeliveryConfig = projectBuilder.addContentDeliveryConfig {
    contentStorage = azureContentStorage.self
    name = "Azure"
  }

  val s3ContentDeliveryConfig = projectBuilder.addContentDeliveryConfig {
  ...
  val automation = projectBuilder.addAutomation {
    ...
    this.actions.add(AutomationAction(this).also { it.contentDeliveryConfig = defaultServerContentDeliveryConfig.self })
  }
