...
interface PermissionRepository : JpaRepository<Permission, Long> {
  ...
  @Query(
    """select p from Permission p
        ...
        left join fetch p.suggestManageLanguages
  ...
  @Query(
    ...
    left join p.suggestManageLanguages sml on sml = :language
    where tl.id is not null or vl.id is not null or scl.id is not null or sul.id is not null or sml.id is not null
  """,
  )
