-- It doesn't need the same filter as backfill because ActivityService.storeActivityData
-- was fixed to never persist a no-op revision - backfill filter out those no-op revisions
