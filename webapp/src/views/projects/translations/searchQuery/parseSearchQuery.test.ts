...
describe('parseSearchQuery', () => {
  it('parses plain text as text tokens', () => {
    ...
    expect(result.hasScopedTokens).toBe(false);
    expect(result.tokens).toEqual([
      { type: 'text', raw: 'hello', value: 'hello' },
      { type: 'text', raw: 'world', value: 'world' },
    ]);
  });
  ...
  it.each(['key', 'description', 'namespace', 'translation'])(
    ...
    (qualifier) => {
      ...
      expect(result.tokens).toEqual([
        { type: 'scoped', qualifier, negated: false, value: 'foo' },
      ]);
    }
  );
  ...
  it('treats unterminated quote as running to the end', () => {
    ...
    expect(result.tokens).toEqual([
      { type: 'scoped', qualifier: 'key', negated: false, value: 'a b c' },
    ]);
  });
  ...
  it('unquotes bare quoted phrases', () => {
    ...
    expect(result.tokens).toEqual([
      { type: 'text', raw: '"two words"', value: 'two words' },
    ]);
  });
  ...
  it('keeps colons inside the value', () => {
    ...
    expect(result.tokens).toEqual([
      { type: 'scoped', qualifier: 'key', negated: false, value: 'foo:bar' },
    ]);
  });

  it('prefers reserved qualifier over language tag', () => {
    const result = parseSearchQuery('translation:foo', ['translation']);
    expect(result.tokens[0]).toMatchObject({ qualifier: 'translation' });
    expect(result.tokens[0]).not.toHaveProperty('languageTag');
  });
