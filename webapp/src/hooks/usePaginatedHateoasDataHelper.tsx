...
export type EmbeddedDataItem<ActionsType extends AbstractLoadableActions<StateWithLoadables<ActionsType>>,
    LoadableName extends keyof ActionsType["loadableDefinitions"]> =
    NonNullable<StateWithLoadables<ActionsType>["loadables"][LoadableName]["data"]> extends HateoasPaginatedData<infer ItemDataType>
        ? ItemDataType : never


export const usePaginatedHateoasDataHelper = <ActionsType extends AbstractLoadableActions<StateWithLoadables<ActionsType>>,
    LoadableName extends keyof ActionsType["loadableDefinitions"]>(
    options: {
        actions: ActionsType,
    ...
    const loadable = options.actions.useSelector((state) => state.loadables[options.loadableName])
    const [currentPage, setCurrentPage] = useState(1)
    const [error, setError] = useState(false);

    const loadPage = (page: number) => {
        const [requestParam, ...otherParams] = options.dispatchParams ? [...options.dispatchParams] : [];
        ...
        }, ...otherParams] as Parameters<(typeof options.actions.loadableActions[LoadableName])["dispatch"]>

        return options.actions.loadableActions[options.loadableName].dispatch(...params)
    }
