...
export class RepositoryPermissions {
  ...
  satisfiesPermission(type: RepositoryPermissionType) {
    const requiredPower = RepositoryPermissions.ORDERED_PERMISSIONS.findIndex(
      (p) => p === type
    );
    const activePower = RepositoryPermissions.ORDERED_PERMISSIONS.findIndex(
...
export const useRepositoryPermissions = (): RepositoryPermissions => {
  ...
  const type =
    RepositoryPermissionType[
      repository.computedPermissions as NonNullable<
        typeof repository.computedPermissions
      >
    ];
