...
// The dotted underline advertises a tooltip, so it only appears when there is one.
const StyledLabelWithHint = styled(StyledLabel)({
...
type AppChipsProps = {
  ...
  /** Renders the caption on its own line, chips underneath. */
  stacked?: boolean;
};
...
export const AppChips = ({
  ...
  const Wrapper = stacked ? StyledStack : StyledRow;

  const caption =
    label &&
    (tooltip ? (
      <Tooltip title={tooltip}>
        <StyledLabelWithHint>{label}</StyledLabelWithHint>
      </Tooltip>
    ) : (
