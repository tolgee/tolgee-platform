...
export const LazyLoadOnScroll: FunctionComponent<{
    ...
    const onScroll = () => {
        ...
        if (wrapper && loadingDiv) {
            const minimalScrollTop = wrapper.scrollTop + wrapper.clientHeight + loadingDiv.clientHeight
    ...
    return (
        <div style={{
            ...
            overflowY: "auto",
            overflowX: "hidden"
        }} ref={wrapperRef} onScroll={onScroll}>
            <Box>
                {props.children}
                <div ref={loadingDivRef}>
                    {props.isMore && <BoxLoading p={2}/>}
                </div>
            </Box>
        </div>
    )
