...
type EmbeddedDataItem<
  ResourceActionsStateType extends StateWithLoadables<any>,
  LoadableName extends keyof ResourceActionsStateType['loadables']
> = NonNullable<
  ResourceActionsStateType['loadables'][LoadableName]['data']
> extends HateoasListData<infer ItemDataType>
  ? ItemDataType
  : never;
...
> {
  renderItem: (
    itemData: EmbeddedDataItem<ResourceActionsStateType, LoadableName>
  ) => ReactNode;
...
export function SimpleHateoasList<
  ItemDataType,
  ActionsType extends AbstractLoadableActions<ResourceActionsStateType>,
  ResourceActionsStateType extends StateWithLoadables<any>,
  LoadableName extends keyof ActionsType['loadableActions']
>(
  props: SimpleHateoasListProps<
    ItemDataType,
    ActionsType,
    ResourceActionsStateType,
    LoadableName
  >
) {
  const loadable = props.actions.useSelector(
    (state) => state.loadables[props.loadableName]
  );

  useEffect(() => {
    if (!loadable.touched) {
      const dispatchParams = props.dispatchParams
        ? [...props.dispatchParams]
        : [];
      return (props.actions.loadableActions as any)[
        props.loadableName
