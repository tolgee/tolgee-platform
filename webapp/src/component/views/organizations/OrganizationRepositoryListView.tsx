...
export const OrganizationsRepositoryListView = () => {
  ...
  const loadable = actions.useSelector(
    (state) => state.loadables.listRepositories
  );

  return (
    <BaseView
      title={t('organization_repositories_title', { name: organization.name })}
    ...
    >
      <SimplePaginatedHateoasList
        ...
        actions={actions}
        searchField
        loadableName="listRepositories"
        renderItem={(r) => <RepositoryListItem {...r} />}
      />
      <Box
        display="flex"
