...
const UserOrganizationSettingsSubtitleLink = (props: UserOrganizationSettingsSubtitleLinkProps) => {
    ...
    const MenuItems = () => {
        ...
        const organizationsLoadable = actions.useSelector(state => state.loadables.listPermittedForMenu)
        ...
        organizationsLoadable.data?._embedded?.organizations?.map(i => {
            data.push({name: i.name, linkTo: LINKS.ORGANIZATION_PROFILE.build({[PARAMS.ORGANIZATION_ADDRESS_PART]: i.addressPart})})
        })
            ...
            <Box display="flex">
                <Link style={{
                    ...
                    flexShrink: 1
                      ...
                      onClick={handleClick}
                >{props.isUser ? <T>user_account_subtitle</T> : <T>organization_account_subtitle</T>}
                    <ArrowDropDown fontSize={"small"}/></Link>
