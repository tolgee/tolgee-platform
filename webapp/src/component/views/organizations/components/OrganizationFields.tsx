...
export const OrganizationFields = () => {
    ...
    useEffect(() => {
        ...
        if (nameChanged) {
            const initialAddressPart = formik.getFieldMeta("addressPart").initialValue;
            const addressPartNotTouchedOrEmpty = !formik.getFieldMeta("addressPart").touched || addressPartValue === ""
            ...
            if (formik.getFieldMeta("name").error == undefined && value != "" && addressPartNotTouchedOrEmpty) {
                organizationService.generateAddressPart(value, initialAddressPart as string).then((addressPart) => {
    ...
    return <>
        ...
        >
            ...
            <FormHelperText><T parameters={{
                address: LINKS.ORGANIZATION.buildWithOrigin({[PARAMS.ORGANIZATION_ADDRESS_PART]: addressPartValue})
            }
