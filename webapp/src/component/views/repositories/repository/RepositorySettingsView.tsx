...
export const RepositorySettingsView: FunctionComponent = () => {
  const loadable = useSelector(
    (state: AppState) => state.repositories.loadables.editRepository
  );
  ...
  let confirm = (options: ConfirmationDialogProps) =>
    confirmation({ title: <T>delete_repository_dialog_title</T>, ...options });
  ...
  return (
    <BaseFormView
      lg={6}
      md={8}
      title={<T>repository_settings_title</T>}
      ...
      customActions={
        <Button
          ...
          onClick={() => {
            confirm({
              message: (
                <T parameters={{ name: repository.name }}>
                  delete_repository_confirmation_message
                </T>
              ),
              onConfirm: () =>
                actions.loadableActions.deleteRepository.dispatch(
                  repository.id
                ),
        ...
        >
          <T>delete_repository_button</T>
        </Button>
      }
    >
      <TextField
        label={<T>repository_settings_name_label</T>}
        name="name"
        required={true}
      />
    </BaseFormView>
